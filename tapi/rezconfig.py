...
# packages_path = [
#     "${SPIN_REZ_LOCAL_ROOT}",     # locally installed pkgs, not yet deployed.
#     "${SPIN_REZ_PIPELINE_ROOT}",  # internally developed pipeline packages.
#     "${SPIN_REZ_OPENSOURCE_ROOT}", # open source packages, such as USD.
#     "${SPIN_REZ_THIRDPARTY_ROOT}", # 3rd party packages, such as houdini, maya.
#     "${SPIN_REZ_PYTHON_PKGS_ROOT}",# pip installed python packages, like numpy.
#     "${SPIN_REZ_MICROSERVICES_ROOT}", # microservice packages.
#     "${SPIN_REZ_IMPLICIT_ROOT}",  # implicit rez packages, such as arch, os.
#     "${SPIN_REZ_PKG_TEMPLATES_ROOT}",  # pkg_templates for rez packages.
# ]

# The path that Rez will locally install packages to when rez-build is used
# local_packages_path = "${SPIN_REZ_LOCAL_ROOT}"
...
# release_packages_path =  "${SPIN_REZ_PIPELINE_ROOT}"
...
#
# package_definition_build_python_paths = ["${SPIN_SW_BOOTSTRAP_ROOT}/src/python/rezutils"]
...
# plugin_path = ["${SPIN_REZ_PLUGINS_PATH}"]
...
#
package_preprocess_function = "build.release_directories"
...
pip_install_remaps = [
    ...
    {
        "record_path": r"{pardir}{sep}{pardir}{sep}share{sep}",
        "pip_install": r"share{s}",
        "rez_install": r"python{s}share{s}",
    }, 
    {
        "record_path": r"{pardir}{sep}{pardir}{sep}etc{sep}",
        "pip_install": r"etc{s}",
        "rez_install": r"python{s}etc{s}",
    },
    {
        "record_path": r"{pardir}{sep}{pardir}{sep}LICENSE",
        "pip_install": r"LICENSE",
        "rez_install": r"LICENSE",
    },
    {
        "record_path": r"{pardir}{sep}{pardir}{sep}include{sep}python{sep}greenlet{sep}greenlet.h",
        "pip_install": r"include{s}python{s}greenlet{s}greenlet.h",
        "rez_install": r"include{s}python{s}greenlet{s}greenlet.h",
    }, 
]
